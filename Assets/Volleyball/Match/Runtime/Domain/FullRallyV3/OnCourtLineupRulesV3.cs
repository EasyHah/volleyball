...
namespace Volleyball.Match.Domain.FullRallyV3
{
    public static class OnCourtLineupRulesV3
    {
            ...
            IReadOnlyList<LiberoReplacementV3> liberoReplacements)
        {
            ...
            var replacementsByLibero =
                ValidateV3Replacements(context, liberoReplacements);
            ...
            AddV3SidePlayers(
                ...
                homeRotationOrder,
                homeServer,
                replacementsByLibero,
                players,
                nameof(homeRotationOrder),
                nameof(homeServer));
            AddV3SidePlayers(
                context.Away,
                TeamSide.Away,
        ...
        private static Dictionary<PlayerId, PlayerId> ValidateV3Replacements(
            MatchContextV3 context,
            IReadOnlyList<LiberoReplacementV3> replacements)
        {
            var playersById = BuildV3PlayerLookup(context.Home, context.Away);
            var replacementsByLibero = new Dictionary<PlayerId, PlayerId>();
            var replacedPlayerIds = new HashSet<PlayerId>();
            for (var index = 0; index < replacements.Count; index++)
            {
                ...
                if (replacement == null)
                {
                    throw new ArgumentException(
                        "Libero replacements cannot contain null.",
                        nameof(replacements));
                }
                ...
                {
                    throw new ArgumentException(
                        "Libero replacements must reference players in the match context.",
                        nameof(replacements));
                }
                ...
                {
                    throw new ArgumentException(
                        "Only a registered libero can make a libero replacement.",
                        nameof(replacements));
                }

                if (replacedPlayer.Position == PlayerPosition.Libero ||
                    SideForV3(context, replacement.LiberoPlayerId) !=
                    SideForV3(context, replacement.ReplacedPlayerId))
                {
                    throw new ArgumentException(
                        "A libero replacement must replace a non-libero teammate.",
                        nameof(replacements));
                }

                if (!replacementsByLibero.TryAdd(
                        replacement.LiberoPlayerId,
                        replacement.ReplacedPlayerId) ||
                    !replacedPlayerIds.Add(replacement.ReplacedPlayerId))
                {
                    throw new ArgumentException(
                        "Each libero and replaced player can appear in only one replacement.",
                        nameof(replacements));
                }
            }
        ...
        private static void AddV3SidePlayers(
            TeamSnapshotV3 team,
            TeamSide side,
            IReadOnlyList<PlayerId> rotationOrder,
            PlayerId server,
            ...
            string serverParamName)
        {
            ...
            {
                throw new ArgumentException(
                    "Exactly six rotation IDs are required.",
                    rotationParamName);
            }

            PlayerWorldSnapshotV3.RequirePlayerId(server, serverParamName);
            var rosterById = BuildV3PlayerLookup(team);
            var rotationIds = new HashSet<PlayerId>();
            for (var index = 0; index < rotationOrder.Count; index++)
            {
                var playerId = PlayerWorldSnapshotV3.RequirePlayerId(
                    rotationOrder[index],
                    rotationParamName);
                if (!rotationIds.Add(playerId))
                {
                    throw new ArgumentException(
                        "Rotation IDs must be distinct.",
                        rotationParamName);
                }
                ...
                {
                    throw new ArgumentException(
                        "Rotation IDs must belong to the matching team.",
                        rotationParamName);
                }

                if (replacementsByLibero.TryGetValue(
                        playerId,
                        out var replacedPlayerId) &&
                    rotationIds.Contains(replacedPlayerId))
                {
                    throw new ArgumentException(
                        "A libero and the player it replaces cannot both be on court.",
                        rotationParamName);
                }
                ...
                players.Add(new OnCourtPlayerEligibilityV3(
                    ...
                    player.Position,
                    index + 1,
                    playerId.Equals(server),
                    replacementsByLibero.TryGetValue(
                        playerId,
                        out replacedPlayerId)
                        ? replacedPlayerId
                        : (PlayerId?)null));
            }
            ...
            foreach (var replacement in replacementsByLibero)
            {
                if (rotationIds.Contains(replacement.Key) &&
                    rotationIds.Contains(replacement.Value))
                {
                    throw new ArgumentException(
                        "A libero and the player it replaces cannot both be on court.",
                        rotationParamName);
            ...
            {
                throw new ArgumentException(
                    "The current server must be in its supplied rotation.",
                    serverParamName);
            }
        }

        private static Dictionary<PlayerId, PlayerSnapshotV3> BuildV3PlayerLookup(
            TeamSnapshotV3 team)
        {
        ...
        private static Dictionary<PlayerId, PlayerSnapshotV3> BuildV3PlayerLookup(
            ...
            TeamSnapshotV3 away)
        {
            var playersById = BuildV3PlayerLookup(home);
            for (var index = 0; index < away.Players.Count; index++)
            {
                var player = away.Players[index];
                playersById.Add(player.PlayerId, player);
        ...
        private static TeamSide SideForV3(
            MatchContextV3 context,
            PlayerId playerId)
        {
            return BuildV3PlayerLookup(context.Home).ContainsKey(playerId)
                ? TeamSide.Home
                : TeamSide.Away;
        }
    }
