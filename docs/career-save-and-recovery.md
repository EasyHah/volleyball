...
> 2026-07-27 实施补充：存档 Schema 仍为 V2，但比赛载荷版本与存档版本已经解耦。
> 新建 `PendingMatch` 只写 `MatchContextV3`，结算历史只写与其匹配的 `MatchResultV3`；
> V3 规范字节、上下文哈希和结果哈希由现有 `Volleyball.Shared` 负责。旧开发档若不含
> 比赛载荷仍可读取；若含已废弃的并行 Match V2 待比赛或历史载荷，则按现有损坏候选
> 隔离/备份恢复流程处理，不伪造迁移结果。详见
> [CHG-20260727-001](changes/2026-07-27-001-career-shared-v3-integration.md)。
