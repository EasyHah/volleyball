...
> 2026-07-28 实施补充：存档 Schema 仍为 V2，但比赛载荷版本与存档版本已经解耦。
> 新建 `PendingMatch` 只写原生 `MatchContextV4`，结算历史只写与其匹配的
> `MatchResultV4`；规范字节、上下文哈希和结果哈希由 `Volleyball.Shared` 负责。
> 旧开发档若不含比赛载荷仍可读取；若含 V1–V3 待比赛或历史载荷，则按现有损坏候选
> 隔离/备份恢复流程处理，不伪造迁移结果。
