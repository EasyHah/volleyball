...
V5-A 已在 `codex/career-match-v5-a` 实现：Career UI 提供独立 V5 profile 的
十二项明确输入和“创建或恢复 V5 正式赛”入口；Bootstrap 创建原生 V5 lifecycle，
并将 profile 与 canonical pending context 持久化。Formal 6v6 在场景启动前校验
冻结的 physics hash，并将 V5 dominant hand 消费到扣球与发球掌面。V4 存档仍是
历史恢复边界，未被填充或迁移为 V5 数据。
