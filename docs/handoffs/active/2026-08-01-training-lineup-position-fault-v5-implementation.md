...
## Windows x64 IL2CPP Player 验收清单

在 Windows 环境上按以下清单补齐 TrainingLab 的平台人工验收；每项据实记录，不允许仅凭自动测试或 macOS 证据
声明通过。验收期间不改动任何逻辑、场景或构建设置——这是纯验证任务。

### 前置条件

- Windows 主机；本机安装 Unity `6000.3.20f1`，且已安装 **Windows Build Support (IL2CPP)** 模块
  （Unity Hub 中对应编辑器版本的 Add Modules）。
- 拉取本阶段分支，`git diff --check` 通过；记录本次验收所在提交 hash。

### 构建

- 在 Editor 菜单执行 `Volleyball/Match/Build training lab Windows x64 Development`。
- 构建成功标志：菜单项所在菜单不再报错，且控制台出现 `Training lab Windows Development Build succeeded`。
- 产物应为 `Builds/Windows/VolleyballTrainingLab.exe`。
- 构建清单应为 `Builds/Windows/training-lab-build-manifest.json`，并核对其中 `unityVersion`、`target`
  （`StandaloneWindows64`）、`architecture`（`x86_64`）、`scriptingBackend`（`IL2CPP`）、`development`（`true`）、
  `scenes`（Training 与 Formal 两场景）与 `executable` 字段与本次构建一致。

### TrainingLab 人工检查

逐项在 Windows Player 中操作并记录通过/失败：

- 轮转锁定：先完成轮转位次编辑并锁定，确认锁定后无法再改动轮转；锁定流程与 macOS Editor 行为一致。
- 拖拽 2D token：在战术板上拖拽一名球员 token，确认站位坐标随拖拽更新且不会误改未操作的坐标轴。
- 构造位置错误：人为制造严格位置反转（如 `Slot2BehindSlot1 is reversed`），确认出现红 token、
  关系连线、纠正箭头与对应文本，且 Serve setup（发球设置）被禁用。
- 纠正：恢复合法站位后，确认红色标识消失、关系连线/纠正箭头清空，Serve setup 重新可用。
- 三正交编辑：分别编辑发球球的 XY 速度与 XZ 位置，确认未编辑的轴保持原值（隐藏轴不被改动）。
- 自由 3D 观察：观察视图可轨道旋转、平移与滚轮缩放；书签保存/重载后视图与当前草稿均未被意外改写。
- 合法情景验证/运行：通过预检的合法轮转与站位可进入运行，且比赛按预期推进。

### V5 生命周期 / V4 pending 恢复

按 `docs/handoffs/deferred/2026-08-01-career-match-v5-windows-validation.md` 的交付物与验收条件执行并记录：

- Windows 上 focused/受影响模块回归、V5 Formal 成功/取消/启动失败/固定种子重放。
- V5 创建/恢复、物理比赛、报告结算闭环；确认 V4 pending 被可恢复地拒绝（明确报错，不静默迁移、不丢数据）。

### 必填记录字段

验收记录至少包含：OS 版本、Unity 版本、提交 hash、manifest/构建报告、Player log 与异常（含路径）、
每一项人工检查的独立通过/失败标记。

### 失败处理

- 任何一项失败：记录可复现步骤与对应日志/截图；handoff 保持 `Status: active`，不宣称完成。
- 仅当失败暴露真实缺陷时才可另行激活修复任务；验收任务本身不改逻辑。
- 全部通过后：更新本文件 Status、结束日期与 Windows 验收证据，移入 `docs/handoffs/completed/`（不得删除）。
