...
- 比分 11:11，蓝队已触球 1 次；这一次是拦网触球，按本简化比赛规则计入三次
  触球。
- 球反弹到本方 `left_front`，高 1.3 米，距网 0.4 米。
- Attacker 刚落地，距球 0.5 米但身体失衡；Setter 距球 0.9 米且面向边线；
  Defender 距离 2.4 米。
- 对方拦网仍停留在左侧。
...
- Attacker `emergency_save` 到 `middle_front`，由 Setter 完成第三次过网。
- Setter 直接 `free_ball` 到对方 `right_back`，风险 `safe`，放弃第三次强攻。
...
需要在仅剩两次触球、近网和球员失衡条件下保住回合，同时把球从对方左侧拦网
转移出去。
