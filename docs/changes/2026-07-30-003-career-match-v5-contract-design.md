...
> `MatchPerformanceReportV1` 补齐真实逐球员事实，再实现 V5 七项核心能力和 readiness。

## 改动摘要

- 盘点 V4 临时属性映射和 `PlayerMatchStatsV4` 输出缺口。
- 定义独立逐球员事实报告的最小字段、来源、不变量、哈希绑定与原子结算边界。
- 参考同类经理游戏的信息分层，并结合完整阵容的 Match 消费字段，将八项明示属性调整为六项动作
  技术和一项运动能力。
- 增加独立二传能力，避免继续使用接发代填二传；定义六项技术到对应比赛动作的直接映射。
- 定义 `Athleticism` 到移动、起跳和协调的单向映射，以及旧八项存档到七项能力的确定性迁移候选。
- 明确原始疲劳、培养方向、潜力、信任、心态与伤病的模块所有权。
- 规定 `Athleticism` 不得替代技术能力；疲劳继续通过 readiness 进入 Match，不在本版伪造耐力消费者。
