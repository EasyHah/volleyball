...
- 根据联调要求，改为 Career 直接保存身体六项和技术九项基础属性，Shared 同名传输。
- Match 集中派生进攻、拦网、防守、接发、二传、发球六类实际比赛属性。
- 定义身体、技术与比赛属性的六维展示；所有雷达汇总只用于 UI，不参与模拟与结算。
...
- [ ] 用户或双方负责人确认两层基础属性、六类比赛属性与六维展示基线。
