...
> `MatchPerformanceReportV1` 补齐真实逐球员事实，再实现 V5 两层基础属性直传、六类比赛属性派生
> 和 readiness。
...
- 根据联调要求，改为 Career 直接保存身体七项和技术八项基础明细，Shared 同名传输。
- 将进攻力量改为身体层的通用力量，将场地意识明确为球商。
- Match 保留防守和接发的独立派生字段，比赛六维维持进攻、拦网、防守、接发、二传、发球。
- 身体六维展示将身高与站立摸高合并为身体尺寸；所有雷达汇总只用于 UI，不参与模拟与结算。
- 定义旧八项存档到两层基础属性的确定性迁移候选。
- 明确原始疲劳、培养方向、潜力、信任、心态与伤病的模块所有权。
- 疲劳继续通过 readiness 进入 Match，身高、站立摸高和身份资料不受 readiness 缩放。
