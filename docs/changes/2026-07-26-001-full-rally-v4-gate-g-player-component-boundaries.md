...
- 关联提交或 PR：`9b4e491` 至 `a82ff4d`（Gate G Task 1–6）、`4a28a6d` 至 `28a0706`（Task 7 验证、边界修复与最终集成修复）
...
- Surface 历史隔离：alignment probe 使用无历史采样，不再提前改写正式 contact surface 的 previous frame；正式接触速度、6v6 对称校准和 3v3 物理路径因此保持基线行为。
...
- 修复后完整 EditMode（HEAD `28a0706`）：`TestResults/GateG-final-verified-editmode.xml`，`588/588` passed，`0` failed/skipped/inconclusive，`5.1556501s`。
- 修复后完整 PlayMode（HEAD `28a0706`）：`TestResults/GateG-final-verified-playmode.xml`，`30/30` passed，`0` failed/skipped/inconclusive，`540.8745213s`。
...
未执行人工场景浏览。最终完整 EditMode/PlayMode 均在修复后的同一 HEAD `28a0706` 上生成通过 XML。

## 回滚与风险

后续风险是再次把 alignment 从 locomotion-owned state 退回 transform-only correction、把 `MaximumSpeed` 重新用作追赶式裁剪器，或让 alignment probe 写入正式 surface history。以速度上界、`0.18m` correction、provider previous-frame、6v6 对称校准和重复 3v3 测试监测。若回滚最终集成修复，应整体评估 `d362e5c..28a0706`；若回滚整个 Gate G，请整体回滚 `9b4e491..28a0706`。
